...
type ServerSession struct {
	// UserIdentifier identifies the user this session belongs to
	UserIdentifier string `json:"identifier"`
...
type SessionKey struct {
	...
	// UserIdentifier identifies the user this key belongs to
...
type SuccessfulAttestationCtx struct {
	UserIdentifier string

}
type SuccessfulAssertionCtx struct {}

type AttestationInitiationCtx struct {}

type AttestationSuccessFunc func(
	http.ResponseWriter, *http.Request, *RegistrationRequest
) error

type AuthenticationSuccessFunc func(
	http.ResponseWriter, *http.Request, *SuccessfulAssertionCtx
) error

type AssertionInitiationFunc func(
	http.ResponseWriter, *http.Request, *AttestationInitiationCtx
)

func (m *Manager) AttestationHandlerFunc(h AttestationSuccessFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var req RegistrationRequest // TODO: AttestationRequest. More generic.
		...
		// TODO: Actually pull Register in here.

		resp, err := m.Register(context.Background(), &req)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}

		if err := h(w, r, &req); err != nil {
...
func (m *Manager) AssertionBeginHandler(h AssertionInitiationFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
			return
		...
		// TODO: Pull BeginAuthentication in here.

		resp, err := m.BeginAuthentication(context.Background(), &req)
		if err != nil {
...
func (m *Manager) AssertionFinishHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
			return
		...
		// TODO: Pull FinishAuthentication in here.

		resp, sessionKey, err := m.FinishAuthentication(context.Background(), &req)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
