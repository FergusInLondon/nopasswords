...
func NewMultiLogger(loggers ...events.EventLogger) *MultiLogger {
	return &MultiLogger{
		loggers: loggers,
	}
}
...
func (m *MultiLogger) AddLogger(logger events.EventLogger) {
	m.loggers = append(m.loggers, logger)
}

// Log implements events.AuditLogger by calling all underlying loggers.
// Errors from individual loggers are collected and returned as a combined error.
func (m *MultiLogger) Log(ctx context.Context, event events.AuditEvent) error {
	var errors []error
...
//	    baseLogger,
//	    memory.ExcludeEventTypeFilter(events.EventTokenGenerate),
//	)
...
type FilterFunc func(event events.AuditEvent) bool

// NewFilteredLogger creates a new filtered logger.
func NewFilteredLogger(logger events.EventLogger, filter FilterFunc) *FilteredLogger {
...
// Log implements events.AuditLogger by filtering events before logging.
func (f *FilteredLogger) Log(ctx context.Context, event events.AuditEvent) error {
	if f.filter(event) {
...
func EventTypeFilter(eventTypes ...string) FilterFunc {
	...
	return func(event events.AuditEvent) bool {
...
func NewAsyncLogger(logger events.EventLogger, bufferSize int) *AsyncLogger {
	return &AsyncLogger{
		logger: logger,
		events: make(chan events.AuditEvent, bufferSize),
		done:   make(chan struct{}),
	}
}

// Log implements events.AuditLogger by sending the event to a channel.
func (a *AsyncLogger) Log(ctx context.Context, event events.AuditEvent) error {
	a.mu.Lock()
	if !a.started {
		a.start()
	}
